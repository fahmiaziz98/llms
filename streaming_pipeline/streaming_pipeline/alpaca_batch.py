...
def build_aplaca_client(
    ...
    if api_key is None:
        try:
            ...
            raise KeyError(
                "WARNING! ALPACA_API_KEY must be seta as env!"
            )
    ...
    if api_secret is None:
        try:
            ...
            raise KeyError(
                "WARNING! ALPACA_SECRET_KEY must be seta as env!"
            )
    
    if tickers is None:
        tickers = ["*"]   

    return AlpacaNewsBatch(
...
class AlpacaNewsBatchSource(StatelessSource):
    ...
    def __init__(
        ...
        self._alpaca_client = build_aplaca_client(
